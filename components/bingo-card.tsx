...
export function BingoCard() {
  ...
  if (stats.count === 0) {
    const onCreate = () => {
      const items: BingoItem[] = BINGO_PRESETS.map((label, i) => ({
        id: newId(),
        label,
        position: i,
      }));
  ...
  return (
    <Pressable onPress={() => router.push('/bingo')} className="active:opacity-90">
          ...
          <View className="flex-1">
            ...
            {stats.linesDone > 0 && (
              <Text className="mt-2 text-sm text-accent-deep">
                {stats.linesDone} ligne{stats.linesDone > 1 ? 's' : ''} complétée
                {stats.linesDone > 1 ? 's' : ''}
              </Text>
            )}
          </View>
