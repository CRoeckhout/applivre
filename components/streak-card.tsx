...
export function StreakCard({ onLongPress, isDragging = false }: StreakCardProps = {}) {
  ...
  const stats = useMemo(() => computeStreaks(completed, today), [completed, today]);
    ...
    if (todayManual) {
      return (
        <Pressable
          ...
          <Text className="text-lg">🔥</Text>
          <Text className="font-sans-med text-paper">Validé — tap pour retirer</Text>
        </Pressable>
      );
    }
    return (
  ...
  return (
    <>
      <Pressable
        ...
        <Animated.View
          ...
          <View className="h-10 w-10 items-center justify-center rounded-full bg-accent">
            <MaterialIcons name="local-fire-department" size={22} color="#fbf8f4" />
          </View>
          <View className="flex-1">
            <Text className="font-display text-xl text-ink">Défi quotidien</Text>
          ...
          {strip.map((day) => {
            ...
            return (
              <Pressable
                ...
                <Text className={`text-xs ${isToday ? 'text-ink' : 'text-ink-muted'}`}>
                  {frShortWeekday(day)}
                </Text>
                <View
        ...
        <View className="mt-5 flex-row gap-3">
          <View className="flex-1 items-center rounded-2xl bg-paper p-3">
            <Text className="font-display text-2xl text-ink">{stats.current}</Text>
            <Text className="mt-0.5 text-xs text-ink-muted">Série actuelle</Text>
          </View>
          <View className="flex-1 items-center rounded-2xl bg-paper p-3">
            <Text className="font-display text-2xl text-ink">{stats.best}</Text>
            <Text className="mt-0.5 text-xs text-ink-muted">Record</Text>
          </View>
          <View className="flex-1 items-center rounded-2xl bg-paper p-3">
            <Text className="font-display text-2xl text-ink">{completed.size}</Text>
      ...
      <StreakSettingsModal open={settingsOpen} onClose={() => setSettingsOpen(false)} />
    </>
  );
}

function StreakSettingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const goalMinutes = usePreferences((s) => s.dailyReadingGoalMinutes);
    ...
    <Modal visible={open} transparent animationType="fade" onRequestClose={onClose}>
      <Pressable
        ...
        <Pressable className="rounded-3xl bg-paper p-6" onPress={(e) => e.stopPropagation()}>
          <Text className="font-display text-2xl text-ink">Objectif quotidien</Text>
          ...
          <View className="mt-5 flex-row items-center justify-center gap-4">
            ...
            <View className="min-w-28 items-center rounded-2xl bg-paper-warm px-4 py-3">
              <TextInput
                ...
                selectTextOnFocus
              />
              <Text className="mt-0.5 text-xs uppercase tracking-wider text-ink-muted">minutes</Text>
            </View>
            <StepperButton onPress={() => adjust(+1)}>+</StepperButton>
            <StepperButton onPress={() => adjust(+5)}>+5</StepperButton>
          </View>
          ...
          <View className="mt-5 flex-row flex-wrap justify-center gap-2">
            {PRESETS_MINUTES.map((p) => (
              <Pressable
                ...
                <Text className={parseInt(value, 10) === p ? 'text-paper' : 'text-ink'}>
                  {p} min
                </Text>
              </Pressable>
            ))}
