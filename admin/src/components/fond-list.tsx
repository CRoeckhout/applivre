...
export function FondList({
  ...
  const filtered = fonds.filter((f) => {
    ...
    const isActive =
      f.retired_at === null &&
      (f.active_from === null || new Date(f.active_from) <= now) &&
      (f.active_until === null || new Date(f.active_until) >= now);
  ...
  return (
    <aside style={{ width: 320, borderRight: '1px solid var(--line)', overflow: 'auto', background: 'var(--surface)' }}>
      ...
      <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
        {filtered.map((f) => {
          const isRetired = f.retired_at !== null;
          const selected = selectedKey === f.fond_key;
        ...
        {filtered.length === 0 && (
          <li style={{ padding: 24, textAlign: 'center' }} className="muted">Aucun fond</li>
        )}
      </ul>
    </aside>
  );
