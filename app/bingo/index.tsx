...
function makePresetItems(): BingoItem[] {
  return BINGO_PRESETS.map((label, i) => ({
    id: newId(),
    label,
    position: i,
  }));
...
function BingoRow({
  ...
  const completions = useBingos((s) => s.completions[bingo.id]) ?? EMPTY_COMPLETIONS;
  ...
  return (
    <Animated.View entering={FadeIn.duration(300).delay(delay)}>
      <Swipeable
        ...
        renderLeftActions={
          archived ? undefined : () => <ArchiveAction onPress={confirmArchive} />
        }
