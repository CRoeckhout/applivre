  ...
  return (
    <SafeAreaView className="flex-1 bg-paper" edges={['top']}>
      <ScrollView contentContainerClassName="px-6 pt-4 pb-24">
        ...
        <Section title="Livres que j'ai prêtés" empty="Tu n'as prêté aucun livre actuellement.">
          {lent.map((e) => (
            <LoanRow key={e.loan.id} entry={e} />
          ))}
        </Section>

        <Section title="Livres empruntés" empty="Tu n'as emprunté aucun livre actuellement.">
          {borrowed.map((e) => (
            <LoanRow key={e.loan.id} entry={e} />
          ))}
        </Section>
...
function SyncSection() {
  ...
  const [state, setState] = useState<'idle' | 'flushing' | 'pushing' | 'pulling'>('idle');
  ...
  const doPush = async () => {
    ...
    try {
      ...
      setFeedback(
        parts.length ? `Envoyé : ${parts.join(', ')}${skippedMsg}` : 'Rien à synchroniser.',
      );
    } catch (e) {
  ...
  return (
    <View className="mt-10">
      <Text className="mb-1 font-display text-xl text-ink">Synchronisation</Text>
      ...
      <View className="gap-2">
        {pendingCount > 0 && (
          <Pressable
            ...
            {state === 'flushing' && <ActivityIndicator size="small" color="#fbf8f4" />}
            <Text className={`font-sans-med ${busy ? 'text-ink-muted' : 'text-paper'}`}>
              Renvoyer maintenant
            </Text>
          </Pressable>
        )}
        ...
        <Pressable
          ...
          {state === 'pulling' && <ActivityIndicator size="small" color="#6b6259" />}
        ...
        <Pressable
          ...
          onPress={doPush}
          className="flex-row items-center justify-center gap-2 rounded-full border border-ink-muted/30 py-3 active:opacity-70">
          {state === 'pushing' && <ActivityIndicator size="small" color="#6b6259" />}
          <Text className="text-ink-soft">Forcer l&apos;envoi local</Text>
        </Pressable>
      </View>

      {feedback && (
        <Text className="mt-3 text-center text-xs text-ink-muted">{feedback}</Text>
      )}
    </View>
  );
}
...
function LoanRow({ entry }: { entry: EnrichedLoan }) {
  ...
  const days = Math.max(0, Math.floor((Date.now() - new Date(loan.dateOut).getTime()) / 86400000));
    ...
    <Pressable
      ...
      <View className="flex-1">
        ...
        <Text className="text-xs text-ink-muted">
          {days === 0 ? "aujourd'hui" : `depuis ${days} jour${days > 1 ? 's' : ''}`}
        </Text>
      </View>
    </Pressable>
  );
